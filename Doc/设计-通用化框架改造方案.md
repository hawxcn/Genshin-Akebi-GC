...
- **`runtime/adapters/*` 与 `games/*`（特定对抗）**：针对具体反作弊系统（如某 UE5 游戏的 EAC/BattlEye、或 IL2CPP 特有检测）的对抗留在对应层，作为可插拔策略挂到 core 的 protection 框架上。
...
// framework.manifest.json（草图，示例为首个 UE5 目标）
{
  "target": {
    ...
    "reparentToExplorer": false,      // UE5 单机通常不需要，按目标反作弊形态决定
    "inject": "ManualMap"             // 或 "LoadLibrary"；反作弊强的目标倾向 ManualMap
  },
  "runtime": "unreal",                // 选择引擎适配器：unity-il2cpp | unreal
  "render":  "auto",                  // auto | d3d11 | d3d12（vulkan 预留，未实现）
  "protection": {                     // 内建反作弊对抗，逐项开关（§5.5）
    "debuggerBypass": true,
    "unlinkModule":   true,
    "eraseHeaders":   true,
    "hookIntegrity":  false
...
### 后续待明确（不阻塞动工，可在 P3 前补齐）

- **首个 UE5 目标的具体游戏名**：影响 SDK 生成的具体符号与偏移、以及是否存在强反作弊（决定 §8.3 的 `inject`/`protection` 取值）。当前设计对具体游戏无硬依赖，可先按通用 UE5 推进 P0–P2。
- **目标是否带强反作弊（EAC/BattlEye 等）**：决定 §5.5 特定对抗策略的投入与注入方式（倾向 ManualMap）。
